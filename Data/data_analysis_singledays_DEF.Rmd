...
```
```{r echo=FALSE}
load("~/Documents/GitHub/BayesCausalFactor/Data/dataset_chemicals.RData")
```
...
parallel_funct_bart<-function(dataset){
  X_0 <- dataset[dataset$treat==0,31:57]
  X_0$monitor <- 1*(X_0$monitor=="CSN")
  X_0$rural <- 1*(X_0$rural=="Mix") + 2*(X_0$rural=="Urban")
  X_0$aug <- 1*(X_0$month==8)
...
parallel_funct_bcf<-function(dataset, threshold=0){
...
bcf_00 <- parallel_funct_bcf(dataset = def_dataset, threshold=0)
```

```{r echo=FALSE}
load("~/Documents/GitHub/CausalFA/sand/data_singleday_results.RData")
```
...
plot_aspaper_vertical<-function(ce_ourmodel,bart_results,month){
  
  mean_our=apply(ce_ourmodel[, 1:1500],1,median)
CI_our=apply(ce_ourmodel[, 1:1500],1, quantile, probs=c(0.05,0.95))
...
plot(mean_our,27:1, pch=16, col=colors_paper,
     xlim=c(-0.6,0.8),      #c(min(min(CI_our),0), max(CI_our)),
     ylab='', xlab='casusal effects', main=month,
     yaxt = "n",bty = "n", cex=1.2)
segments(CI_our[1,], 27:1, CI_our[2,],27:1, col=colors_paper, lwd = 3)
points( apply(bart_results, 2, mean),27:1+0.3, col="black", cex=0.5)
segments(CI_bart[1,], 27:1+0.3, CI_bart[2,],27:1+0.3, col="black")
...
plot_aspaper_vertical(ce_ourmodel=(model_00_paper$CE_across),
             ...
             month='thr 0.0')
plot_aspaper_vertical(ce_ourmodel=(model_02_paper$CE_across),
             bart_results= (bart_02),
             month='thr 0.2')
plot_aspaper_vertical(ce_ourmodel=(model_05_paper$CE_across),
             bart_results= (bart_05),
             month='thr 0.5')
plot_aspaper_vertical(ce_ourmodel=(model_10_paper$CE_across),
             bart_results= (bart_10),
             month='thr 1.0')
plot_aspaper_vertical(ce_ourmodel=(model_20_paper$CE_across),
             bart_results= (bart_20),
             month='thr 2.0')
plot_aspaper_vertical(ce_ourmodel=(model_50_paper$CE_across),
             bart_results= (bart_50),
             month='thr 5.0')
plot(0:10,0:10,type = 'n', yaxt = "n",bty = "n",xaxt = "n",ylab='',xlab='')
op <- par(cex = 0.5)
legend(-1,5, c("Our model (CRFM):","pos. median","CI 95%"," ","BART:","pos. median","CI 95%"),
       pch=c(NA,19,NA,NA,NA,19,NA), col=c(NA,2,2,NA,NA,1,1), lty=c(NA,NA,1,NA,NA,NA,1), bty = "n")
```



In the paper *The influence of wildfire smoke on ambient chemical species concentrations in the contiguous US*, they obtain the following results:

![](paper_ce.png){width=50%}