...
# setting 8: 
data_8 <- lapply(1:n_sample, function(x) funct_data_generating(
                   n_tot=n_units, p=dim_Y, seed=x, 
                   function_Y=4,
                   mean_cl=mu_eta, c_U=gamma_U, setting_XU=1))
```
...
est_CE_8 <- sapply(data_8,function(x) apply(x$mis_data$Y_all[[2]]-x$mis_data$Y_all[[1]],2,mean))
...
each_CE_plot(est_CE_8, ylim_par=ylim_par,
             name_title="scenario 8", name_y=name_y)
...
true_CE_8 <- apply(est_CE_8,1,mean)
```
...
# setting 8:
model_8_mix <- mclapply(data_8, BayesCausalFactor_parall, mc.cores = 5) 
model_8_stand <- mclapply(data_8, StandardFactor_parall, mc.cores = 5)
model_8_bart <- mclapply(data_8, parallel_funct_bart, mc.cores = 5)
model_8_bcf <- lapply(1:n_sample, function(x) BCF_estimation(Y_t = data_8[[x]]$data$Y_t, 
                                                              X_t = data_8[[x]]$data$X_t, 
                                                              Treat = data_8[[x]]$data$Treat))
...
ITE_data3bis <- lapply(data_3BIS, function(d) simulated_ITE(d))
...
ITE_data7bis <- lapply(data_7BIS, function(d) simulated_ITE(d))
ITE_data8 <- lapply(data_8, function(d) simulated_ITE(d))
...
# setting 3BIS
bias_3bis_mix <- lapply(1:n_sample, function(s) 
  bias_function(ite_simulated = ITE_data3bis[[s]], ite_estimated = model_3BIS_mix[[s]]$CE_ite))
bias_3bis_stand <- lapply(1:n_sample, function(s) 
  bias_function(ite_simulated = ITE_data3bis[[s]], ite_estimated = model_3BIS_stand[[s]]$CE_ite))
bias_3bis_bart <- lapply(1:n_sample, function(s) 
  bias_function(ite_simulated = ITE_data3bis[[s]], 
                ite_estimated = delist_bart_tauY(model_3BIS_bart[[s]])))
bias_3bis_bcf <- lapply(1:n_sample, function(s) 
  bias_function(ite_simulated = ITE_data3bis[[s]], 
                ite_estimated = delist_bcf_tauY(model_3BIS_bcf[[s]][[1]])))
...
# setting 7BIS
bias_7bis_mix <- lapply(1:n_sample, function(s) 
  bias_function(ite_simulated = ITE_data7bis[[s]], ite_estimated = model_7BIS_mix[[s]]$CE_ite))
bias_7bis_stand <- lapply(1:n_sample, function(s) 
  bias_function(ite_simulated = ITE_data7bis[[s]], ite_estimated = model_7BIS_stand[[s]]$CE_ite))
bias_7bis_bart <- lapply(1:n_sample, function(s) 
  bias_function(ite_simulated = ITE_data7bis[[s]], 
                ite_estimated = delist_bart_tauY(model_7BIS_bart[[s]])))
bias_7bis_bcf <- lapply(1:n_sample, function(s) 
  bias_function(ite_simulated = ITE_data7bis[[s]], 
                ite_estimated = delist_bcf_tauY(model_7BIS_bcf[[s]][[1]])))
```
...
boxplot_models_general<-function(data, list_model, xlab_name, ylab_name){
  ...
  ggplot(bias_boxplot, aes(x=cov, y=Xi, fill=Q, color=Q, alpha=0.2)) + 
    ...
    geom_hline(yintercept = 0, col="#AB0600",linetype = "dashed", size=0.4) +
...
boxplot_models_general(data=c(t(sapply(bias_3bis_mix, function(i) unlist(i$bias_CE_mean))),
                              t(sapply(bias_3bis_stand, function(i) unlist(i$bias_CE_mean))),
                              t(sapply(bias_3bis_bart, function(i) unlist(i$bias_CE_mean))),
                              t(sapply(bias_3bis_bcf, function(i) unlist(i$bias_CE_mean)))),
                       list_model=c("CausalFA","StandardFA","bart","BCF"),
                       xlab_name="Scenario 3 BIS", ylab_name="Bias")
boxplot_models_scen4(data=c(t(sapply(bias_4_mix, function(i) unlist(i$bias_CE_mean))),
...
boxplot_models_general(data=c(t(sapply(bias_7bis_mix, function(i) unlist(i$bias_CE_mean))),
                              t(sapply(bias_7bis_stand, function(i) unlist(i$bias_CE_mean))),
                              t(sapply(bias_7bis_bart, function(i) unlist(i$bias_CE_mean))),
                              t(sapply(bias_7bis_bcf, function(i) unlist(i$bias_CE_mean)))),
                       list_model=c("CausalFA","StandardFA","bart","BCF"),
                       xlab_name="Scenario 7BIS", ylab_name="Bias")
...
boxplot_models_general(data=c(t(sapply(bias_3bis_mix, function(i) unlist(i$mse_CE_mean))),
                              t(sapply(bias_3bis_stand, function(i) unlist(i$mse_CE_mean))),
                              t(sapply(bias_3bis_bart, function(i) unlist(i$mse_CE_mean))),
                              t(sapply(bias_3bis_bcf, function(i) unlist(i$mse_CE_mean)))),
                       list_model=c("CausalFA","StandardFA","bart","BCF"),
                       xlab_name="Scenario 3 BIS", ylab_name="MSE")
...
boxplot_models_general(data=c(t(sapply(bias_7bis_mix, function(i) unlist(i$mse_CE_mean))),
                              t(sapply(bias_7bis_stand, function(i) unlist(i$mse_CE_mean))),
                              t(sapply(bias_7bis_bart, function(i) unlist(i$mse_CE_mean))),
                              t(sapply(bias_7bis_bcf, function(i) unlist(i$mse_CE_mean)))),
                       list_model=c("CausalFA","StandardFA","bart","BCF"),
                       xlab_name="Scenario 7BIS", ylab_name="MSE")
```
...
delist_bart_ate <- function(results){
  return(t(sapply(results, function(d) unlist(d$mean_CE))))
}
#prepare BCF data
delist_bcf_ate <- function(results){
  seq_positions<- seq(1,length(results),2)+1
  return(t(sapply(seq_positions, function(d) unlist(results[[d]]))))
}
